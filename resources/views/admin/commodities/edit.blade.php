                    ...
                    <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="allow_installment" id="allow_installment" value="1"
                                       {{ $commodity->allow_installment ? 'checked' : '' }}
                        ...
                        <div id="installment_options" class="pl-6 space-y-4 {{ $commodity->allow_installment ? '' : 'hidden' }}">
                            <div>
                                <label for="max_installment_months" class="block text-sm font-medium text-gray-700">
                                    Maximum Installment Period (Months)
                                </label>
                                <input type="number" name="max_installment_months" id="max_installment_months" min="1" max="36"
                                       value="{{ $commodity->max_installment_months ?? 3 }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="initial_deposit_percentage" id="initial_deposit_percentage" min="0" max="100" step="5"
                                       value="{{ $commodity->initial_deposit_percentage ?? 20 }}"
                                       class="mt-1 focus:ring-purple-500 focus:border-purple-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                <p class="mt-1 text-xs text-gray-500">
                            ...
                            <div id="payment_preview" class="mt-4 p-4 bg-white border border-gray-200 rounded-md {{ $commodity->allow_installment ? '' : 'hidden' }}">
                                <h4 class="text-sm font-medium text-gray-700 mb-2">Payment Preview</h4>
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between">
                                        <span>Total Price:</span>
                                        <span id="preview_total_price">₦{{ number_format($commodity->price, 2) }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Initial Deposit:</span>
                                        <span id="preview_initial_deposit">₦{{ number_format(($commodity->price * ($commodity->initial_deposit_percentage ?? 0)) / 100, 2) }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span>Remaining Amount:</span>
                                        <span id="preview_remaining">₦{{ number_format($commodity->price - (($commodity->price * ($commodity->initial_deposit_percentage ?? 0)) / 100), 2) }}</span>
                                    </div>
                                    <div class="flex justify-between font-medium">
                                        <span>Monthly Payment:</span>
                                        <span id="preview_monthly">₦{{ number_format($commodity->monthly_installment_amount ?? 0, 2) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
